...
app.get("/getAll", async (req, res) => {
  ...
  if (process.env.PROD) res.json({ msg: 'you arent supposed to be here...' });
...
const uploadPdf = async (buff, accessToken, title, spSiteName) => {
  ...
  if (!siteRes.ok || !siteData.value || siteData.value.length === 0) {
    throw new Error(
      `Could not find site with title "${spSiteName}": ${JSON.stringify(siteData)}`,
    );
  }
  ...
  const uploadRes = await fetch(
    `https://graph.microsoft.com/v1.0/sites/${siteId}/drives/${driveId}/root:/${title}.pdf:/content`,
    {
      method: "PUT",
      headers: {
        Authorization: `Bearer ${accessToken}`,
