...
router.get('/getSheet', async (req, res) => {
  try {
    ...
    if (!resp.ok) {
      throw new Error(
        `Smartsheet API error: ${resp.status} ${resp.statusText}`
      );
    }
    ...
    const projectType = idx('Project Type');
    ...
    const cells = data.rows
      .map(item => {
        ...
        if (windowStart && new Date(endDate) < new Date(windowStart))
          return null;
